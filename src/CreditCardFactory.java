	...
	public static CreditCard makeCreditCard(String cardNumber, String expirationDate, String holderName) {
		...
		if (isValidCardNumber(cardNumber)) {
			if (isMasterCard(cardNumber)) {
				return new MasterCard(cardNumber, expirationDate, holderName);
			}
			if (isVisaCard(cardNumber)) {
				return new VisaCard(cardNumber, expirationDate, holderName);
			}
			if (isAmericanExpress(cardNumber)) {
				return new AmericanExpress(cardNumber, expirationDate, holderName);
			}
			if (isDiscoverCard(cardNumber)) {
				return new DiscoverCard(cardNumber, expirationDate, holderName);
			}
		}
...
//		System.out.println(cardNumber.substring(1, 2));
		int integerSecondDigit = Integer.parseInt(cardNumber.substring(1, 2));
